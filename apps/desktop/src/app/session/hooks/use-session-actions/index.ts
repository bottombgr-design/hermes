...
import { hasSessionInfoStatePatch, sessionInfoStatePatch } from '../use-message-stream/utils'
  ...
  const forkBranch = useCallback(
    async (
      ...
      try {
        ...
        // The branch opens in an isolated tile; applying runtime info through
        // the global helper would silently repaint the foreground chat's model,
        // cwd, and controls. Hydrate only the child runtime state.
        const runtimeInfo = sessionInfoStatePatch(branched.info)
        patchSessionWorkspace(routedSessionId, runtimeInfo.cwd)

        if (hasSessionInfoStatePatch(runtimeInfo)) {
