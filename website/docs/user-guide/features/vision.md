...
Vision providers generally accept JPEG, PNG, GIF, and WebP pixels but reject raw SVG/XML payloads. Hermes accepts self-contained SVG input, rejects active content and external references before rendering, converts it to a temporary PNG, and sends that PNG through either the native or auxiliary vision path. The source SVG is never modified.

Hermes tries CairoSVG, svglib with ReportLab, `rsvg-convert`, Inkscape, and the built-in macOS Quick Look renderer in that order. If none is available, convert the SVG to PNG before calling `vision_analyze`.
