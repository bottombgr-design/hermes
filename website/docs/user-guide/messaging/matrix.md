...
Matrix edits improve the normal client view, but every `m.replace` is still a room event. Clients, bridges, and notification pipelines that do not consistently aggregate replacement events can expose each update in history or notifications. This is why progress is quiet by default even though the opt-in path normally edits one visible message.
