...
| `DISCORD_ALLOW_BOTS` | No | `"none"` | Controls how the bot handles messages from other Discord bots. `"none"` — ignore all other bots. `"mentions"` — only accept bot messages that `@mention` Hermes. `"all"` — accept all bot messages. Overrides `config.yaml` `discord.allow_bots`. Accepts input from a single trusted relay/webhook bot only — see the bot-to-bot warning below before changing it from `"none"`. |
