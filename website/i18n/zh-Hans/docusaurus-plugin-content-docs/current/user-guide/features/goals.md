...
| `/goal --file <path>` | 从 UTF-8 文件加载目标文本，而无需粘贴。**仅限 CLI、TUI 和 Desktop** ——消息 gateway 不可用。见[从文件加载目标](#从文件加载目标)。 |
...
长目标难以粘贴进 TUI 或 CLI。`/goal --file <path>` 改为从 UTF-8 文件读取目标文本：
...
- **路径在 Hermes 后端主机上读取**，使用 Hermes 进程用户。在 TUI 和 Desktop 中，相对路径基于活动会话的工作目录解析；在 Classic CLI 中，基于进程工作目录。`~` 展开为后端进程用户的 home 目录。
...
`/goal --file` **仅**在本地 CLI、TUI 和 Desktop 中可用。在消息 gateway（Telegram、Discord、Slack 等）上会返回不支持的错误，且绝不读取文件——这一边界防止远程聊天用户读取 Hermes 后端主机上的任意文件。
