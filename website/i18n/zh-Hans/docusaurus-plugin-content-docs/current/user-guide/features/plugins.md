...
| [`transform_tool_result`](/user-guide/features/hooks#transform_tool_result) | 任意工具返回后、结果进入模型上下文前。返回字符串可替换结果。 |
| [`transform_terminal_output`](/user-guide/features/hooks#transform_terminal_output) | 终端输出被截断、剥离 ANSI 序列和脱敏前。返回字符串可替换输出。 |
| [`transform_llm_output`](/user-guide/features/hooks#transform_llm_output) | 最终响应生成后、交付给用户前。返回字符串可替换响应。 |
