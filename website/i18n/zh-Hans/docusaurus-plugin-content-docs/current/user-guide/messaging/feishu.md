...
在飞书开发者后台的 **权限管理** 中，至少开通接收消息、以机器人身份发送消息、读取资源和读取会话信息所需权限。然后在 **事件与回调** 中选择 **长连接（WebSocket）**（推荐）或配置 webhook，并订阅 `im.message.receive_v1`。

### 在独立话题中开始任务

在飞书/Lark 私聊中，如果一项工作需要独立的会话和完整记录，可以使用 `/thread <任务>`：

```text
/thread 帮我整理电脑上的下载文件
```

Hermes 会在该命令下创建一个新的飞书话题，并只把 `/thread` 后面的文字作为任务执行。生成的 `thread_id` 会成为 Hermes 会话键的一部分，因此进度消息、最终结果以及你之后在话题里的回复都会复用同一个隔离会话，不会继续使用主私聊会话。如果话题创建失败，Hermes 会在主聊天中明确提示，并且不会退回主聊天执行任务。

`/thread` 适合明确划分任务边界。普通单轮问题仍留在主聊天；系统不会自动把任意消息升级为话题，因为在执行前无法可靠判断任务是否一定需要多轮互动。

### 机器人自定义菜单命令

Hermes 可以把飞书/Lark 机器人自定义菜单的点击事件转换为普通入站文本消息。这适合把 `/status`、`/help` 或 `/new` 等命令放进菜单，用户无需手动输入。

1. 在开发者后台启用机器人的 **自定义菜单** 能力。
2. 新建菜单项，动作类型选择 **推送事件**。
3. 将菜单项的事件 key 设置为 Hermes 应收到的完整文本，例如 `/status`。
4. 在 **事件与回调** 中订阅 `application.bot.menu_v6`（机器人自定义菜单事件）。
5. 无需额外配置 home chat。Hermes 会使用点击者的 Open ID，将事件路由到该用户自己的 P2P 会话。
6. 保存菜单与事件订阅后，创建并发布新的应用版本。

用户点击菜单后，Hermes 会解析点击者身份，立即发送一条简短确认（例如“已接收：状态，正在查询…”），然后通过与普通私信相同的准入和处理链路处理事件 key。因此用户白名单和配对机制仍然生效，最终结果也会返回点击者自己的 P2P 聊天。Hermes 使用飞书 event ID 去重，飞书重试投递时不会重复确认或执行命令。如果即时确认发送失败，Hermes 会记录发送错误，但仍继续尝试执行用户请求。

:::tip
需要确定性命令行为时，建议直接使用 slash command 字符串作为事件 key。也可以填写普通文本，Hermes 会像用户在私信中发送该文本一样处理。
:::
