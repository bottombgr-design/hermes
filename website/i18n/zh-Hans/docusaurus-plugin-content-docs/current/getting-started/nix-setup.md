...
`documents` 选项会保留无目录键的现有行为：它们会安装到 `workingDirectory`。包含目录的键是相对于 `${services.hermes-agent.stateDir}`（默认 `/var/lib/hermes`）的显式路径。这样可以在不迁移现有工作区文件的情况下，将文档放到 `$HERMES_HOME` 下。

- 对工作区文件使用 `USER.md` 这类无目录键（兼容旧版）
- 对 Hermes 从 `$HERMES_HOME` 读取的文件，使用 `.hermes/...`
- 其他包含目录的键会按你写出的路径原样放到 `stateDir` 下
...
这样既能保留现有工作区文件的兼容性，也能让嵌套目标路径保持显式：
...
| 选项 | 类型 | 默认值 | 描述 |
|---|---|---|---|
| `documents` | `attrsOf (either str path)` | `{}` | 无目录键安装到 `workingDirectory`；嵌套键安装到 `stateDir` 下，激活时会自动创建父目录 |
