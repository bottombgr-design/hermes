...
export function ToolsetConfigDrawer({
    ...
    const poll = async () => {
      try {
        ...
        if (st.running) {
          ...
          showToast(
            ok
              ? t.toolsetConfig.postSetupComplete
              : t.toolsetConfig.postSetupErrors,
            ok ? "success" : "error",
          );
  ...
  const handleSelectProvider = async (provider: ToolsetProvider) => {
    ...
    try {
      ...
      showToast(
        format(t.toolsetConfig.providerSelected, { provider: provider.name }),
        "success",
      );
      onChanged();
    } catch (e) {
      showToast(
        e instanceof Error ? e.message : t.toolsetConfig.providerSelectFailed,
        "error",
      );
    } finally {
      setSelecting(null);
  ...
  const handleSaveKeys = async (provider: ToolsetProvider) => {
    ...
    try {
      ...
      showToast(
        e instanceof Error ? e.message : t.toolsetConfig.saveKeysFailed,
        "error",
      );
    } finally {
  ...
  const handleRunPostSetup = async (provider: ToolsetProvider) => {
    ...
    try {
      ...
      setPostSetupRunning(false);
      showToast(
        e instanceof Error ? e.message : t.toolsetConfig.postSetupStartFailed,
        "error",
      );
    }
  };
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-5 pt-4 space-y-4">
          {loading ? (
            ...
            <p className="text-sm text-muted-foreground py-6 text-center">
              {t.toolsetConfig.noBackends}
            </p>
          ) : config.providers.length === 0 ? (
            <p className="text-sm text-muted-foreground py-6 text-center">
              {t.toolsetConfig.noProviders}
            </p>
          ) : (
                      ...
                      {provider.env_vars.map((ev) => (
                        <div key={ev.key} className="space-y-1">
                          ...
                          <Input
                            ...
                            placeholder={
                              isSet[ev.key]
                                ? t.toolsetConfig.savedPlaceholder
                                : ev.prompt || ev.key
                            }
                            value={drafts[ev.key] ?? ""}
                            onChange={(e) =>
