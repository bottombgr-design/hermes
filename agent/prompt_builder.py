...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            "## Skills (mandatory)\n"
            "Scan the index before replying. If a skill is relevant or even partially "
            "relevant, you MUST load it with skill_view(name) before acting and follow "
            "it, even when basic tools could handle the task; if uncertain, load it. "
            "Descriptions are trigger hints; full procedures load on demand.\n"
            "For Hermes Agent setup, configuration, modification, or troubleshooting, "
            "load `hermes-agent` first and treat the official docs as authoritative. "
            "Patch a loaded skill when its instructions are stale. After difficult "
            "reusable work, offer to save the procedure as a skill.\n"
            "\n"
            "<available_skills>\n"
            + "\n".join(index_lines) + "\n"
            "</available_skills>\n"
            "\n"
            "Proceed without loading a skill only when none applies."
            + hidden_note
        )
