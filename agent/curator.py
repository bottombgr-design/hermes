...
CURATOR_REVIEW_PROMPT = (
    ...
    "Safe edit protocol — mandatory for every skill_manage call:\n"
    "  - Resolve skill names from the active profile only: use skills_list or "
    "skill_view first. Do not guess names from memory, paths, prior sessions, "
    "or another profile. If skill_view says the skill is missing, skip it and "
    "report the stale name; do not create a lookalike replacement.\n"
    "  - Immediately before every action=patch or action=edit, call skill_view "
    "for the exact target skill in this same run. If you have not loaded that "
    "current SKILL.md, do not patch.\n"
    "  - Patch with a unique, high-context old_string. If skill_manage reports "
    "multiple matches, no match, stale loaded content, or any mismatch, STOP "
    "editing that skill for this run. Do not retry with a broader string. "
    "Record it under leftovers for a human pass.\n"
    "  - Before action=create or action=edit, validate the complete SKILL.md "
    "frontmatter mentally as YAML: start/end --- fences, name + description "
    "present, quote colons or other punctuation in scalar values. If unsure, "
    "do not write; report the draft instead.\n"
    "  - Never use action=write_file or action=remove_file for top-level "
    "SKILL.md. Those actions are only for references/, templates/, scripts/, "
    "or assets/ support files. Use action=patch/edit/create for SKILL.md.\n"
    "  - Treat bundled, hub-installed, external-dir, and protected built-in "
    "skills as read-only even if a previous line made them look tempting. "
    "Do not try write_file, patch, edit, delete, archive, mv, cp, or rm on "
    "them unless the human explicitly approved that exact bundled target.\n\n"
