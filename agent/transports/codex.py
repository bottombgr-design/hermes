...
class ResponsesApiTransport(ProviderTransport):
    ...
    def build_kwargs(
        ...
        **params,
        ...
        #
        # GPT-5 and above support server-side agentic web search on any
        # Responses API endpoint (OpenAI, GitHub Copilot, OpenRouter, custom
        # proxies).  Grok models support the same native built-in on any
        # Responses endpoint (including Copilot); no version gate for Grok,
        # consistent with the xAI-direct block above.
        #
        ...
        if not is_xai_responses and response_tools:
            ...
            if is_grok or model_stem.startswith("gpt-"):
                try:
                    major = int(model_stem.split("-")[1].split(".")[0])
                except (IndexError, ValueError):
                    major = 0
                if is_grok or major >= 5:
                    has_client_web_search = any(
                        isinstance(t, dict) and t.get("name") == "web_search"
                        for t in response_tools
                    )
