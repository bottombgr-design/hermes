...
def _run_approval_gate(
    *,
    ...
    if not is_cli and not is_gateway:
        # Unattended cron and Kanban workers cannot receive a live response,
        # even when they inherited gateway/ask markers from their parent.
        if _is_unattended_approval_context():
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if _is_unattended_approval_context() or (
        not is_cli and not is_gateway and not is_ask
    ):
