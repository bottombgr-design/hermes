...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        if valid_branch.returncode != 0:
            ...
            sys.exit(1)
        original_identity = _capture_update_checkout_identity(git_cmd, PROJECT_ROOT)
        ...
        pinned_target_sha = _git_update_commit_sha(git_cmd, PROJECT_ROOT, remote_ref)
        ...
        if is_fork and branch == "main":
            fork_plan = _pin_fork_upstream_target(
                git_cmd, PROJECT_ROOT, pinned_origin_sha
            )
            if fork_plan["error"]:
                print(f"✗ {fork_plan['error']}")
                print("  No lockfile discard, stash, checkout, reset, or ref update was attempted.")
                sys.exit(1)
            pinned_target_sha = fork_plan["target_sha"]
            fork_sync_needed = fork_plan["sync_needed"]
        pinned_pre_update_sha = _git_update_commit_sha(
            git_cmd, PROJECT_ROOT, branch_ref
        )
        ...
        if pinned_pre_update_sha is not None:
            fast_forward_error = _pinned_fast_forward_error(
                git_cmd, PROJECT_ROOT, pinned_pre_update_sha, pinned_target_sha
            )
            if fast_forward_error:
                print(f"✗ {fast_forward_error}")
                print("  No lockfile discard, stash, checkout, or ref update was attempted.")
                sys.exit(1)

        after_preflight = _capture_update_checkout_identity(git_cmd, PROJECT_ROOT)
        if after_preflight != original_identity:
            print("✗ Checkout changed during update preflight; retry from the desired checkout.")
            sys.exit(1)

        # npm can rewrite tracked lockfiles during local builds. This is the
        # first worktree mutation and is permitted only after the pinned
        # history boundary has been proven.
        _discard_lockfile_churn(git_cmd, PROJECT_ROOT)
        after_lockfile = _capture_update_checkout_identity(git_cmd, PROJECT_ROOT)
        if after_lockfile != original_identity:
            print("✗ Checkout changed during lockfile cleanup; no stash was attempted.")
            sys.exit(1)

        count_result = subprocess.run(
            git_cmd + ["rev-list", f"{history_start_sha}..{pinned_target_sha}", "--count"],
            cwd=PROJECT_ROOT,
            capture_output=True,
            text=True, encoding="utf-8", errors="replace",
            check=True,
        ...
        if not exact_target:
            if original_identity["ref"] != branch_ref:
                ...
                print(f"  ⚠ Currently on {label} — updating {branch} through a pinned checkout...")
            ...
            if fork_sync_needed:
                syntax_ok, failing_path, syntax_error = (
                    _validate_critical_files_syntax_at_commit(
                        git_cmd, PROJECT_ROOT, pinned_target_sha
                    )
                )
                if not syntax_ok:
                    print(f"✗ Refusing to sync fork: critical syntax error in {failing_path}.")
                    if syntax_error:
                        print(f"  {syntax_error}")
                    sys.exit(1)
                print("→ Syncing verified target to fork...")
                if _push_pinned_fork_target(
                    git_cmd,
                    PROJECT_ROOT,
                    pinned_target_sha,
                    pinned_origin_sha,
                    ...
                    print("  ℹ Fork changed concurrently or could not be updated; local checkout is unchanged.")
        ...
        applied = _apply_pinned_default_update(
            git_cmd,
            PROJECT_ROOT,
            branch_ref,
        ...
        if not syntax_ok:
            ...
            if syntax_error:
                for line in str(syntax_error).splitlines()[:6]:
                    print(f"    {line}")
            print()
            print(f"→ Rolling back to {(pinned_pre_update_sha or original_identity['head'])[:10]}...")
            rolled_back = _rollback_pinned_default_update(
                git_cmd,
                PROJECT_ROOT,
                branch_ref,
                pinned_pre_update_sha,
            ...
            if rolled_back:
                ...
                print("  ✗ Rollback stopped because checkout or branch identity changed.")
                if auto_stash_ref is not None:
                    print(f"  Local changes remain preserved in stash {auto_stash_ref}.")
            sys.exit(1)
        ...
        if auto_stash_ref is not None:
            ...
            if discard_local_changes:
                stash_handled = _discard_stashed_changes(
                    ...
                    PROJECT_ROOT,
                    auto_stash_ref,
                    expected_identity={"ref": branch_ref, "head": pinned_target_sha},
                )
            ...
            if stash_handled is False:
                print("✗ Stopping update because local-change recovery was not completed safely.")
                sys.exit(1)
        ...
        if fork_sync_needed:
            ...
            if _push_pinned_fork_target(
                ...
                print("  ℹ Fork changed concurrently or could not be updated; local checkout remains verified.")
