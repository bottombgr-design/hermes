...
# each other. gettempdir() still honours $TMPDIR first on POSIX, so this is
# unchanged there; on Windows the old "/tmp" fallback resolved to a
# non-existent \tmp on the current drive and raised at import time — before
# the skipif below could be evaluated — aborting collection for the whole
# suite.
