...
async def integ(tmp_path, monkeypatch):
    ...
    envs: list = []
    # set_multiplex_active flips a process-global flag; capture it here and
    # restore it in teardown so a multiplexed env built by this factory cannot
    # leak fail-closed get_secret() semantics into unrelated tests that run
    # after this suite (they would start raising UnscopedSecretError).
