...
class TestListCronJobRuns:
    ...
    def test_active_run_excluded_ended_at_required(self, db):
        """An in-flight cron run (ended_at IS NULL) must NOT appear in the
        run-history endpoint — desktop consumers only want *completed* runs.
        Regression for the active-vs-completed listing semantics Teknium's
        hermes-sweeper review asked us to cover (2026-07-25).
        """
        base = 1_700_000_000.0
        completed = self._seed_run(db, "alpha", 0, base)
        # An in-flight run that has not yet ended.
        active_id = f"cron_alpha_{1:08d}"
        db.create_session(session_id=active_id, source="cron")
        db.append_message(active_id, role="user", content=f"run 1 for alpha")
        db._conn.execute(
            "UPDATE sessions SET started_at = ? WHERE id = ?",
            (base + 60, active_id),
        )
        ...
        assert [r["id"] for r in runs] == [completed]
        # Belt-and-suspenders: the active row stays in the DB, just isn't surfaced.
        all_alpha = db._conn.execute(
            "SELECT id FROM sessions WHERE id LIKE 'cron_alpha_%' ORDER BY id"
        ).fetchall()
        assert {r["id"] for r in all_alpha} == {completed, active_id}
