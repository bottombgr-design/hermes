...
class TestFTSExternalContentMigration:
    ...
    def test_optimize_settle_refuses_pending_backfill(self, tmp_path):
        ...
        try:
            ...
            # Wipe indexed rows so step has work, then prevent finish by
            # short-circuiting: leave markers by stubbing finish? Easier —
            # call the settle check path by making step always leave markers.
            # Force incomplete: after one step progress advances but we then
            # re-set markers high so settle refuses... Actually the public
            # contract is optimize returns ok=False when still pending.
            # Simulate by stubbing fts_rebuild_step to no-op while markers stay.
