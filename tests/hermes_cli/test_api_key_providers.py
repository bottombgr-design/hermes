...
class TestResolveApiKeyProviderCredentials:
    ...
    def test_qoder_acp_rejects_unimplemented_tcp_transport(self, monkeypatch):
        monkeypatch.setenv("QODER_ACP_BASE_URL", "acp+tcp://127.0.0.1:8765")
        monkeypatch.setattr("hermes_cli.auth.shutil.which", lambda _command: None)

        status = get_external_process_provider_status("qoder-acp")
        assert status["configured"] is False
        assert status["logged_in"] is False
        assert status["error"] == "unsupported_transport"

        with pytest.raises(AuthError, match="ACP TCP transport is not supported"):
            resolve_external_process_provider_credentials("qoder-acp")
